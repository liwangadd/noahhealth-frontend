...
class SecondCategoryManage extends React.Component {
  ...
  requestSecondCategoryData = (firstId, pageNow) => {
    ...
    if (this.props.params.tabKey === '1' || this.props.params.tabKey === '2') {
      ...
      $.ajax({
        url: SERVER + '/api/health_category/second/' + firstId + '/list',
        type: 'POST',
        contentType: 'application/json',
        dataType: 'json',
        data: JSON.stringify({ pageNow: pageNow, pageSize: PAGE_SIZE }),
  ...
  handleDelete(record) {
    ...
    if (this.props.params.tabKey === '1' || this.props.params.tabKey === '2') {
      ...
      $.ajax({
        url: SERVER + '/api/health_category/second/' + record.id,
        type: 'DELETE',
        dataType: 'json',
        beforeSend: (request) => request.setRequestHeader(SESSION.TOKEN, sessionStorage.getItem(SESSION.TOKEN)),
        success: (result) => {
  ...
  requestCategory = (categoryId) => {
    ...
    if (this.props.params.tabKey === '1' || this.props.params.tabKey === '2') {
      ...
      $.ajax({
        url: SERVER + '/api/health_category/second/' + categoryId,
        type: 'GET',
        dataType: 'json',
        beforeSend: (request) => request.setRequestHeader(SESSION.TOKEN, sessionStorage.getItem(SESSION.TOKEN)),
        success: (result) => {
    ...
    this.refs.editForm.validateFields((err, values) => {
      if (!err) {
        ...
        if (this.props.params.tabKey === '1' || this.props.params.tabKey === '2') {
          ...
          $.ajax({
            url: SERVER + '/api/health_category/second/' + this.categoryId,
            type: 'PUT',
            contentType: 'application/json',
            data: JSON.stringify({ firstId: Number(this.props.params.firstId), name: values.name, number: values.number }),
            dataType: 'json',
