        ...
        if (detail.status === "录入中" || detail.status === "未通过")
            detailOperation =
                <div>
                    ...
                    <Button className="gutter" size="default" onClick={() => this.props.onSave(this.props.form, detail.id, this.state.note)} loading={this.props.saveLoading}>保存</Button>
                    ...
                    }
                    {
                        role === ROLE.EMPLOYEE_ARCHIVE_MANAGER || role === ROLE.EMPLOYEE_ADMIN
                            ...
                            <span>
                                ...
                                <Popconfirm title="您确定要通过审核吗?" placement="bottom" onConfirm={() => this.props.onPass(this.props.form, detail.id, this.state.note)}>
        ...
        return (
            <Timeline.Item dot={timeLineIcon}>
                ...
                <Card title={detail.status} extra={<a onClick={this.switchForm}>{this.state.switchText}</a>} className="exam-result-detail-item-card">
                    <p className={detail.normal === true ? "abnormal" : ""}>检查亚类：{detail.secondName}</p>
