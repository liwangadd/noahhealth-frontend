...
import ExamAssayDetailAddModal from './ExamAssayDetailAddModal.js';
...
class ExamResultAssayManage extends React.Component {
      ...
      if(!err) {
        ...
        $.ajax({
            ...
            data : JSON.stringify({userId: Number(values.userId),
                                   secondId: values.secondCategoryParentOfAssayId[1],
  ...
  render(){
        ...
        <span>
          {
            ...
            ?
            <Link to={ROUTE.EXAM_RESULT_ASSAY_CLOSEUP.URL_PREFIX + "/" + ROUTE.EXAM_RESULT_ASSAY_CLOSEUP.MENU_KEY + "/" + record.userName + "/" + record.id}>{record.status === '未通过' ? '重新录入' : '录入'}</Link>
            :
            null
          }

          {
            ...
            ?
            <Link to={ROUTE.EXAM_RESULT_ASSAY_CLOSEUP.URL_PREFIX + "/" + ROUTE.EXAM_RESULT_ASSAY_CLOSEUP.MENU_KEY + "/" + record.userName + "/" + record.id}>{role === ROLE.EMPLOYEE_ARCHIVER ? '查看' : '审核'}</Link>
            :
            null
          }

          {
            ...
            ?
            <Link to={ROUTE.EXAM_RESULT_ASSAY_CLOSEUP.URL_PREFIX + "/" + ROUTE.EXAM_RESULT_ASSAY_CLOSEUP.MENU_KEY + "/" + record.userName + "/" + record.id}>查看</Link>
            :
            null
          }
    ...
    return (
      <div>
        ...
        <ExamAssayDetailAddModal ref="addForm" visible={this.state.addModalVisible} confirmLoading={this.state.confirmAddModalLoading} onCancel={this.closeAddModal} onConfirm={this.confirmAddModal} 
        memberUnderEmployeeData={this.state.memberUnderEmployeeData} secondCategoryParentOfAssayData={this.state.secondCategoryParentOfAssayData}/>
      </div>
    );
  }
}

export default ExamResultAssayManage;
